// src/app/admin/page.tsx 전체 코드 (updateCount 열 추가)
...
export default function AdminPage() {
  ...
  // --- ▼▼▼ fetchData 함수 수정 (updateCount 기본값 설정) ▼▼▼ ---
  const fetchData = async (password: string): Promise<boolean> => {
    ...
    try {
      ...
      const processedData = json.map(app => ({
        ...app,
        updateCount: app.updateCount ?? 0, // 기본값 0 설정
  ...
  const handleLogin=async(e:FormEvent)=>{e.preventDefault();setIsLoading(true);if(await fetchData(passwordInput))sessionStorage.setItem('admin_password',passwordInput);else{setError('비밀번호가 틀렸거나 데이터를 불러올 수 없습니다.');setPasswordInput('')}setIsLoading(false)};
  const handleUpdate=async(id:string,updates:Partial<Application>)=>{try{const pw=sessionStorage.getItem('admin_password');if(!pw)throw new Error('인증 만료');const res=await fetch('/api/update-application',{method:'PATCH',headers:{'Content-Type':'application/json','Authorization':`Bearer ${pw}`},body:JSON.stringify({id,updates})});if(!res.ok)throw new Error((await res.json()).message);setData(d=>d.map(app=>(app.createdAt===id?{...app,...updates}:app)))}catch(err){alert(`오류: ${err instanceof Error?err.message:'업데이트 실패'}`)}};
  ...
  const sortedData=useMemo(()=>{const items=[...data];if(sortConfig.key){items.sort((a,b)=>{const valA=a[sortConfig.key!];const valB=b[sortConfig.key!];let comp=0;if(typeof valA==='boolean'&&typeof valB==='boolean'){comp=valA===valB?0:valA?-1:1}else if(typeof valA==='string'&&typeof valB==='string'){if(sortConfig.key==='heroPower'){comp=(parseInt(valA.replace(/,/g,''))||0)-(parseInt(valB.replace(/,/g,''))||0)}else{comp=valA.localeCompare(valB)}}else if(typeof valA==='number'&&typeof valB==='number'){comp=valA-valB}return sortConfig.direction==='ascending'?comp:-comp})}return items},[data,sortConfig]);
  ...
  if(!authenticated){return(<main className="max-w-md mx-auto p-6 mt-10 text-center bg-white shadow-lg rounded-lg"><h1 className="text-2xl font-bold mb-4">관리자 로그인</h1><form onSubmit={handleLogin}><input type="password" className="border p-3 w-full mb-4 rounded-md" placeholder="비밀번호" value={passwordInput} onChange={e=>setPasswordInput(e.target.value)} disabled={isLoading}/><button type="submit" className="bg-black text-white px-6 py-3 rounded-md w-full font-semibold" disabled={isLoading}>{isLoading?'확인 중...':'로그인'}</button>{error&&<p className="text-red-500 mt-4">{error}</p>}</form></main>)}

  return (
    <main className="max-w-full mx-auto p-4 sm:p-6">
      <div className="flex justify-between items-center mb-6"><h1 className="text-xl sm:text-3xl font-bold">이민 신청자 목록</h1><button onClick={handleReset} disabled={isLoading} className="bg-red-600 text-white px-4 py-2 rounded-md font-semibold hover:bg-red-700 disabled:bg-gray-400">전체 초기화</button></div>
        ...
        <table className="min-w-[1200px] w-full border-collapse text-xs">
          <thead className="bg-gray-100">
            <tr>
              ...
              {/* --- ▼▼▼ 여기가 수정된 부분입니다 (updateCount 헤더 추가) ▼▼▼ --- */}
              <th className="border-b px-2 py-2 text-center font-medium text-gray-600 w-16"><button onClick={()=>requestSort('updateCount')} className='w-full text-center'>횟수<br/>Upd.{sortConfig.key==='updateCount'?(sortConfig.direction==='ascending'?' ▲':' ▼'):''}</button></th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedData.map(row=>(
              <tr key={row.createdAt} className={`${row.isConfirmed?'bg-green-50':'bg-white'} hover:bg-gray-50`}>
                ...
                {/* --- ▼▼▼ 여기가 수정된 부분입니다 (updateCount 데이터 표시) ▼▼▼ --- */}
                <td className="px-2 py-2 text-center">{row.updateCount > 0 ? <span className="font-bold text-blue-600">{row.updateCount}</span> : row.updateCount}</td>
            ...
            {sortedData.length===0&&<tr><td colSpan={TABLE_COLUMNS.length+5} className="text-center py-10 text-gray-500">데이터가 없습니다.</td></tr>}
          </tbody>
        </table>
      </div>
