// src/app/admin/page.tsx 전체 코드 (수정 이력 기능 최종본)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl">
        <div className="p-4 border-b flex justify-between items-center">
          <h3 className="text-lg font-semibold">'{nickname}' 님의 수정 이력 / History</h3>
        ...
        <div className="p-4 max-h-[70vh] overflow-y-auto">
          <table className="min-w-full text-xs">
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                <th className="p-2 text-left font-semibold">신청일 (Version)</th>
                <th className="p-2 text-left font-semibold">서버/연맹</th>
                <th className="p-2 text-left font-semibold">전투력</th>
                <th className="p-2 text-left font-semibold">주력군종</th>
                <th className="p-2 text-left font-semibold">등급</th>
                <th className="p-2 text-left font-semibold">목표연맹</th>
                <th className="p-2 text-center font-semibold">이미지</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {history.map(app => (
                <tr key={app.createdAt}>
                  <td className="p-2">{new Date(app.createdAt).toLocaleString('ko-KR')}</td>
                  <td className="p-2">{app.currentServerAndAlliance}</td>
                  <td className="p-2">{app.heroPower}</td>
                  <td className="p-2">{app.mainSquad}</td>
                  <td className="p-2">{app.immigrationGrade}</td>
                  <td className="p-2">{app.targetAlliance}</td>
                  <td className="p-2 text-center">{app.image ? <a href={app.image} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">보기</a> : '없음'}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function AdminPage() {
  ...
  const handleLogin = async (event: FormEvent) => {
    ...
    if (await fetchData(passwordInput)) {
  ...
  const handleUpdate = async (applicantId: string, updates: Partial<Application>) => {
    try {
      ...
      // 최신 버전에만 업데이트 반영
      const latestVersion = allApplications
  ...
  const handleReset = async () => { /* 이전과 동일 */ };

  const { displayData, historyCounts } = useMemo(() => {
    const grouped = allApplications.reduce((acc, app) => {
      acc[app.applicantId] = acc[app.applicantId] || [];
      acc[app.applicantId].push(app);
      return acc;
    ...
    for (const id in grouped) {
      const history = grouped[id].sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      display.push(history[0]);
      counts[id] = history.length;
    }
  ...
  const sortedData = useMemo(() => {
    ...
    if (sortConfig.key) {
      items.sort((a, b) => { /* 이전과 동일 */ return 0; });
    }
    return items;
  }, [displayData, sortConfig]);
  
  const requestSort = (key: keyof Application) => { /* 이전과 동일 */ };
        ...
        <table className="min-w-[1200px] w-full border-collapse text-xs">
          <thead className="bg-gray-100">
            <tr>
              ...
              {TABLE_COLUMNS.map(col=>(<th key={col.key} className={`border-b p-2 font-medium text-gray-600 ${col.width} ${col.isNumeric?'text-right':'text-left'}`}><button onClick={()=>requestSort(col.key)} className="w-full h-full text-inherit" dangerouslySetInnerHTML={{__html:col.label+(sortConfig.key===col.key?(sortConfig.direction==='ascending'?' ▲':' ▼'):'')}}/></th>))}
              ...
              return (
                <tr key={row.applicantId} className={`${row.isConfirmed ? 'bg-green-50' : 'bg-white'} hover:bg-gray-50`}>
                  ...
                  <td className="p-2 text-center">
                    {historyCount > 1 ? (
                      <button onClick={() => openHistoryModal(row.applicantId)} className="font-bold text-blue-600 hover:underline">{historyCount}</button>
